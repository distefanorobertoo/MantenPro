...
def import_assets():
    ...
    if file:
        ...
        try:
            ...
            conn.commit()
            flash('Se importaron los datos con exito!', 'success')
    ...
    flash('Formato del archivo invalido', 'error')
    return redirect(url_for('import_assets'))
...
def remove_asset(user_asset_id):
    ...
    try:
        ...
        conn.commit()
        flash('¡El activo ha sido desvinculado del usuario exitosamente!', 'success')
    except Exception as e:
        conn.rollback()
        flash(f'Error: {str(e)}', 'error')
    finally:
...
def remove_user(user_id):
    ...
    try:
        ...
        conn.commit()
        flash('Se elimino el usuario', 'success')
    except Exception as e:
        conn.rollback()
        flash(f'Error: {str(e)}', 'error')
    finally:
...
def add_asset():
    if request.method == 'POST':
        ...
        flash('Se agrego el activo y se asigno al usuario en forma exitosa!', 'success')
        return redirect(url_for('assign_assets'))
...
def edit_asset(asset_id):
    ...
    if request.method == 'POST':
        ...
        flash('Detalles del activo actualizados', 'success')
        return redirect(url_for('view_assets'))
...
def delete_asset(asset_id):
    ...
    try:
        ...
        c.execute('DELETE FROM assets WHERE id = ?', (asset_id,))
        conn.commit()
        flash('Se elimino el activo', 'success')
    except Exception as e:
        conn.rollback()
        flash(f'Error: {str(e)}', 'error')
    finally:
    ...
    if request.method == 'POST':
        ...
        if 'user_file' in request.files:
            ...
            if user_file.filename != '' and allowed_file(user_file.filename):
                ...
                try:
                    ...
                    conn.commit()
                    flash('Se importaron los usuarios', 'success')
        ...
        # Handle single user addition
            ...
            try:
                c.execute('INSERT INTO users (username, email) VALUES (?, ?)', (username, email))
                conn.commit()
                flash('Se agrego el usuario', 'success')
            except Exception as e:
                conn.rollback()
                flash(f'Error: {str(e)}', 'error')
            finally:
...
def edit_user(user_id):
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        conn.close()
        flash('Se actualizo el usuario!', 'success')
...
def import_user_assets():
    ...
    if file:
        ...
        try:
            ...
            conn.commit()
            flash('Se importo los activos', 'success')
        except Exception as e:
            conn.rollback()
            flash(f'Error: {str(e)}', 'error')
        finally:
...
def sql_console():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Se ejecuto con exito la consulta SQL', 'success')
        except Exception as e:
            conn.rollback()
